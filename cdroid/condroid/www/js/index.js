...
var app = {
    ...
    onDeviceReady: function() {
        ...
        //need to test if it works on iOS
...
function pairBluetooth(){
    ...
    //sort this callback out to do something when it fails or fill the list as the devices are discovered
...
function subHandler(data){
    //TODO: clean up this function (switch case and functions)
    var fields = data.split(':'); 

    //showToast(data); //uncomment to show all data that is passing

    //if we recieved IMSI 
    if(fields[0] == "IMSI"){
        var imsi = document.getElementById("imsi");
        imsi.innerHTML = "IMSI: " + fields[1].replace('#', '' ); 
        m_imsi = fields[1].replace('#', '' ); 
    }   
    ...
    //if we recieved anything else 
        ...
        //successful refresh 
        if(fields[1].replace('#', '') == "Refresh successful"){
            attached = true; 
        }

        showToast(fields[1].replace('#', ''));

        //if attach request is unsuccessful 
        if(fields[1].replace('#', '') == "A_FALSE"){
            attached = false; 
            showToast("Attach not successful"); 
        }
        //if it is succesful
        else if(fields[1].replace('#', '') == "Attach successful"){
            attached = true; 
            showToast("Attach successful"); 
        }

        //if a detach request is unsuccessful 
        if(fields[1].replace('#', '') == "D_FALSE"){
            attached = true; 
        }

        //if a detach request is succesful 
        else if(fields[1].replace('#', '') == "Detach successful"){
            attached = false; 
            emptyProgressbar();
        }
...
function detachRequest(){
    bluetoothSerial.isConnected(function(){
        if(!attached){
            showToast("Already detached. Try to attach to network first."); 
         }
         else{
             var DETACHREQUEST = '5'; 
...
$(document).ready(function(){
    ...
    $('#scanBtn').click(function(){ pairBluetooth(); return false; });
    ...
    setInterval(function() {
        $('#date-part').html(new Date().toDateString() + " " + new Date().toTimeString().replace("GMT+0100", "").replace("(centraleuropeisk normaltid)", "").replace("(CET)", ""));
